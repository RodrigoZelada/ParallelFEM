void laplace2D(double* A, int* I, int* J, double* b, mesh &Th, double* F, int totalnodes, int mynode, mesh &ThGlobal, int &s, int* localToGlobalVertices)
{
	int Nv = Th.nv;
	int Nt = Th.nt;
	...
	for (n = 0; n < Nt; n++){
		...
		for (i = 0; i <= D; i++){
			...
			// ii_global = 0;
			// while ( (Th.vertices[ii][0] != ThGlobal.vertices[ii_global][0]) || (Th.vertices[ii][1] != ThGlobal.vertices[ii_global][1])){
			// 	ii_global++;
			// }
			ii_global = localToGlobalVertices[ii];

			b[ii_global] += (area/3)*dot(coefii,Pe.row(i))*F[ii];
			...
			for (j = 0; j <= D; j++){
				...
				// jj_global = 0;
				// while ( (Th.vertices[jj][0] != ThGlobal.vertices[jj_global][0]) || (Th.vertices[jj][1] != ThGlobal.vertices[jj_global][1])){
				// 	jj_global++;
				// }
				jj_global = localToGlobalVertices[jj];

				//a(u,v) = grad(u)*grad(v), u = aii + bii*x + cii*y
				k=0;
				if (s>0){
					while ((k<s) && ((I[k] != ii_global) || (J[k] != jj_global))){
						k++;
					}
					if (k==s){
						I[s] = ii_global;
